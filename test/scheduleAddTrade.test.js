/* Add a NEW trade to an active schedule with chosen dependencies (the Task-list
 * "Add trade" flow). Mirrors the POST /:sid/items transaction: insert item +
 * deps[] (predecessors) + dependents[] (successors) → recompute → materialize a
 * linked calendar task. Verifies mid-chain placement cascades + shifts
 * downstream, the trade becomes a real linked task, and a cycle rolls back.
 * Real MySQL. Run: NODE_PATH=<backend>/node_modules node test/scheduleAddTrade.test.js
 */
'use strict';
...
(async () => {
  ...
  try {
    ...
    // Mirror POST /:sid/items with deps/dependents + materialize, in a transaction.
    async function addTrade({ name, dur, deps = [], dependents = [], assignee = null }) {
      ...
      try {
        ...
        for (const d of deps) await conn.query('INSERT IGNORE INTO job_schedule_deps (schedule_id,item_id,depends_on_item_id) VALUES (?,?,?)', [sid, iid, d]);
        for (const dep of dependents) await conn.query('INSERT IGNORE INTO job_schedule_deps (schedule_id,item_id,depends_on_item_id) VALUES (?,?,?)', [sid, dep, iid]);
    ...
    const A = await itemByName('A'), B = await itemByName('B'), C = await itemByName('C');
    const cStartBefore = ymd(C.computed_start_date);

    // Add D (dur 8) depends on A, and make C depend on D (mid-chain insert).
    const dId = await addTrade({ name: 'D Rough-in', dur: 8, deps: [A.id], dependents: [C.id], assignee: 55 });
    const D = await itemByName('D'); const C2 = await itemByName('C');
    ...
    ok(ymd(C2.computed_start_date) > ymd(D.computed_end_date), 'add: C (made to depend on D) now starts after D finishes', ymd(C2.computed_start_date) + ' vs D end ' + ymd(D.computed_end_date));
    ok(ymd(C2.computed_start_date) > cStartBefore, 'add: downstream C shifted LATER than before', cStartBefore + ' → ' + ymd(C2.computed_start_date));
    ...
    ok(dTask && Number(dTask.is_calendar_task) === 1 && Number(dTask.user_id) === 55, 'add: D task is a calendar task with the chosen assignee', JSON.stringify(dTask));
    ok((await liveTasks()) === 4, 'add: 4 live calendar tasks now', String(await liveTasks()));

    // Cycle: add E depends on C but make A depend on E → A→…→E→C→…→A loop → rejected, rolled back.
    let threw = null;
    try { await addTrade({ name: 'E Loop', dur: 1, deps: [C.id], dependents: [A.id] }); } catch (e) { threw = e; }
    ok(threw && (threw.code === 'CYCLE' || threw.cycle), 'cycle: a self-referential add is rejected (CYCLE)', threw ? (threw.code || 'err') : 'no throw');
    const eItem = await itemByName('E');
    ok(!eItem, 'cycle: the rejected trade was rolled back (no orphan item)', eItem ? 'orphan present' : '');
    ok((await liveTasks()) === 4, 'cycle: still 4 live tasks (no orphan task)', String(await liveTasks()));
